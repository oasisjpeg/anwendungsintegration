...
export function ArticleDetailModal({ article, quiz, onClose }: ArticleDetail201ModalProps) {
  ...
  // Reset quiz state when a new article/quiz is loaded
  useEffect(() => {
    if (article && quiz) {
      console.log(`Loading new quiz for article ${article.id}, quiz ${quiz.id}`);
  ...
  }, [article?.id, quiz?.id, openModal, closeModal]);

  // If no article or quiz, don't render anything
  ...
  const submitAnswer = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${getBaseUrl()}/api/articles/${article.id}/quiz/submissions`,
        requestBody,
        {
          headers: {
  ...
  return (
    <div className="fixed inset-0 bg-black flex items-start justify-center z-50 px-4 pb-[calc(env(safe-area-inset-bottom,0)+5rem)]" onClick={(e) => {
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 max-w-md w-full max-h-[85vh] overflow-y-auto mt-[calc(env(safe-area-inset-top,0)+2.5rem)] mb-[calc(env(safe-area-inset-top,0)+2.5rem)]" onClick={(e) => e.stopPropagation()}>
        <div className="relative">
          ...
          <h2 className="text-2xl font-semibold mb-4 pr-8">{article.title}</h2>
          <p className="text-gray-600 dark:text-gray-300 mb-2">{article.description}</p>
          {article.url && article.url.length > 0 && (
            <img
              src={article.url[0]}
              alt={article.title}
              className="rounded-lg object-cover mb-4 w-full max-h-64"
            />
          )}
          ...
          <div className="mt-6">
            ...
            {!showScore ? (
              ...
              <button
                ...
                className="mt-4 bg-indigo-600 dark:bg-indigo-500 text-white font-semibold rounded-xl px-4 py-2 text-sm shadow-md hover:bg-indigo-700 disabled:opacity-50"
              >
